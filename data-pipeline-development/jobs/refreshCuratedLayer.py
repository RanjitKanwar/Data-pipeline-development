...
def refreshCuratedLayerTable(entity:str,
                             sf_proc:str,
                             process_execution_id:int,
                             job_execution_id:int ):
    refreshCuratedLayerTableStatus=True
    try :

        logger.info(f'{entity} : Refresh curated layer start')
        ...
        if len(result_list)>0:
            ...
            if job_execution_details_df['status'].str.contains('Failed').any():
                refreshCuratedLayerTableStatus=False
            ...
            logger.info(f'{entity} : Refresh curated layer ended successfully')

    except Exception as e:
        logger.error(f'{entity} : Refresh curated layer  failed')
        logger.error(e)
        refreshCuratedLayerTableStatus=False

    return refreshCuratedLayerTableStatus


def main(  process_execution_id:int,
            job_execution_id:int,
           sf_proc:str
    ):
    job_status=True
    ...
    thread_result_set={}
    history_entity_list =metastoreFunc.get_history_entity_list(process_execution_id)
    for entity in history_entity_list:
        ...
        thread_result_set[entity] =THREAD_POOL.apply_async(refreshCuratedLayerTable,kwds=arg_dict)
